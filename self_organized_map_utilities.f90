...
!
!    do ix=1,nx
!       write(*,*) (kohonen_map%grid_pattern_index(ix,iy,1),iy=1,ny)
!    enddo
!    stop
...
!*****
   ...
   integer :: kneigh,idbg,number_variables !neff,
...
!========================================================================================
   class(self_organized_map) :: kohonen_map
   real(kind=8),dimension(:,:),intent(inout) :: count_
!*****
...
!****f* self_organized_map_utilities/update_weights1
! NAME
!   update_weights1
! PURPOSE
!    Subroutine to update the weights   
! SYNOPSIS
!========================================================================================
!  subroutine update_weights1(kohonen_map,current_values,ihit,jhit,khit,&
!             maximum_radius,iteration,distances) 
! !========================================================================================
!   class(self_organized_map) :: kohonen_map
!   real(kind=8),dimension(:,:),intent(inout) :: current_values,distances
!   integer,intent(inout) :: ihit,jhit,khit,iteration
!   real(kind=8),intent(inout) :: maximum_radius
! !*****
!   real(kind=8),dimension(size(current_values,1),size(current_values,2)) :: prototype_values
...
!               call kohonen_map%grid(ineigh,jneigh,kneigh)%get_prototype(prototype_values);
!               !prototype_values=prototype_values+h_neighborhood*(current_values-prototype_values);
!               sample1=kohonen_map%grid_pattern_index(ineigh,jneigh,kneigh);
!               sample2=kohonen_map%grid_pattern_index(ihit,jhit,khit);
!               !write(100,*) ic,ineigh,jneigh,kneigh,sample2,sample1
!               prototype_values=prototype_values+h_neighborhood*(current_values-prototype_values);!+dsqrt(distances(sample2,sample1)); !
!               call kohonen_map%grid(ineigh,jneigh,kneigh)%set_prototype(prototype_values)
!            case('visom')
!               !write(*,*) trim(kohonen_map%parameters%som_type)
!               call kohonen_map%grid(ineigh,jneigh,kneigh)%get_prototype(prototype_values);
...
!               sample1=kohonen_map%grid_pattern_index(ineigh,jneigh,kneigh);
!               sample2=kohonen_map%grid_pattern_index(ihit,jhit,khit);
!               !real_distance=distances(sample2,sample1);
!               if( (ineigh .eq. ihit) .and. (jneigh .eq. jhit) .and. (kneigh .eq. khit) ) then                           
!                  prototype_values=prototype_values+h_neighborhood*(current_values-prototype_values);
!               else
!                  distance_ratio=dsqrt(real_distance)/(dsqrt(geometric_distance2)*lambda);
...
!    enddo!ic
! 
!  end subroutine update_weights1 
...
!     do iy=1,size(kohonen_map%grid,2);
!        do ix=1,size(kohonen_map%grid,1);
!           dist_min=1.0d10;ihit=0;jhit=0;khit=0;
!           do idat=1,size(input_data)
!              call input_data(idat)%get(current_prototype);
!              dist=kohonen_map%grid(ix,iy,iz)%distance(current_prototype,kohonen_map%distance_function);
!              if(dist < dist_min) then 
...
!
!  do ix=1,size(kohonen_map%grid,1)
!     write(100,'(20I5)') (kohonen_map%grid_pattern_index(ix,iy,1),iy=1,size(kohonen_map%grid,2))
!  enddo
!
...
! Extract results
 ...
 enddo
!
! write(*,*) 'distortion= ',allocated(my_som%distortion)
...
!    do inode=1,(nx*ny)
!       dist_hit=0.0d0;
!       dist_hit=sum((new_pat(ipat,:)-prot(inode,:))**2)/dble(nvar)
!       if(dist_hit < dist) then
!          dist=dist_hit
!          i_hit=inode;
!       endif
!    enddo
!    call index2position(i_hit,nx1,ny1,nz1,cx,cy,cz);
!    node_index(ipat,1)=cx;
!    node_index(ipat,2)=cy;
!    node_index(ipat,3)=cz;
!  enddo!ipat
!
